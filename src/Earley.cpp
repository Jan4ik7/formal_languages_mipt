...
EarleyParser::Situation::Situation() {}
...
EarleyParser::EarleyParser(Grammar_CF &grammar)
    : start_rule_(Grammar_CF::Rule(0, {grammar.GetStartSymbol()})),
      grammar_(grammar) {}

std::vector<std::vector<EarleyParser::Situation>>
EarleyParser::Initialise(std::string &word) {
...
void EarleyParser::Scan(size_t k, size_t j, std::string &word,
  ...
  if (S[k][j].Next() == grammar_.GetEpsilon()) {
